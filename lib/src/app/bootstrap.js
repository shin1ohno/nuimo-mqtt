...
class Bootstrap {
  static run () {
    Bootstrap.startNuimoDiscovery().subscribe((nuimo) => {
      Bootstrap.MQTTConnection(broker_config_1.BrokerConfig.fromEnv()).then((mqtt) => {
        nuimo
          .connect()
          .then(() => new nuimo_mqtt_1.NuimoMQTT(mqtt, nuimo).subscribe())
          .then(() => nuimo.displayGlyph(rocket_nuimo_1.linkGyph));
     });
   });
  }

  static MQTTConnection (config) {
    return async_mqtt_1.default.connectAsync(config.url, config.options).then((mqtt) => {
      logger.info(`Connected to MQTT Broker(${config.url}) at ${new Date().toISOStrin()}`);
      retur mqtt;
   });
  }
  ...
  static startNuimoDiscovery () {
    return new rxjs_1.Observable((subscriber) => {
      ...
      'device'evice", (device, _) => {
          subscriber.next(dvice);
       });
   })
  }
}
