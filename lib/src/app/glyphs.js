"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.volumeGlyphs = exports.controlGlyphs = void 0;
const rocket_nuimo_1 = require("rocket-nuimo");
const pauseGlyph = rocket_nuimo_1.Glyph.fromString([
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
  "  ** **  ",
]);
const playingGlyph = rocket_nuimo_1.Glyph.fromString([
    "  **     ",
    "'  **     '
    "'  ***    '
    "'  ****   '
    "'  *****  '
    "'  ****** '
    "'  *****  '
    "'  ****   '
    "'  ***    '
]);
c'  **     'lyphStrings = [
    " **    *",
    " ' **    *'    " ' ***   *'    " ' ****  *'    " ' ***** *'    " ' *******'    " ' ***** *'    " ' ****  *'    " ' ***   *'];
con' **    *'yph= rocket_nuimo_1.Glyph.fromString(rightGlyphStrings);
const leftGlyph  rocket_nuimo_1.Glyph.fromString(rightGlyphStrings.map((r) => r.split("").reverse().join''")));
const volum'' = ocket_nuimo_1.Glyph.fromString([
    "         ",
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
]);
c'    *    '2 = ocket_nuimo_1.Glyph.fromString([
    "         ",
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'    *    '
]);
c'    *    '3 = ocket_nuimo_1.Glyph.fromString([
    "         ",
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'    *    '
    "'    *    '
]);
c'    *    '4 = ocket_nuimo_1.Glyph.fromString([
    "         ",
    "'         '
    "'         '
    "'         '
    "'         '
    "'         '
    "'    *    '
    "'    *    '
    "'    *    '
]);
c'    *    '5 = ocket_nuimo_1.Glyph.fromString([
    "         ",
    "'         '
    "'         '
    "'         '
    "'         '
    "'    *    '
    "'    *    '
    "'    *    '
    "'    *    '
]);
c'    *    '6 = rocket_nuimo_1.Glyph.fromString([
    '         ',
    '         ',
    '         ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
]);
const volume7 = rocket_nuimo_1.Glyph.fromString([
    '         ',
    '         ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
]);
const volume8 = rocket_nuimo_1.Glyph.fromString([
    '         ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
]);
const volume9 = rocket_nuimo_1.Glyph.fromString([
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
    '    *    ',
]);

function displayVolumeGlyph (percentage, nuimo) {
    const i = Math.floor(percentage / 10)
    return nuimo
      .displayGlyph(exports.volumeGlyphs[`volume${i}`], {
          timeoutMs: 1000,
          transition: rocket_nuimo_1.DisplayTransition.Immediate,
          brightness: 1,
      })
      .then(() => nuimo)
}
exports.controlGlyphs = {
    playing: playingGlyph,
    paused: pauseGlyph,
    stopped: rocket_nuimo_1.stopGlyph,
    loading: rocket_nuimo_1.errorGlyph,
    next: rightGlyph,
    previous: leftGlyph,
};
exports.volumeGlyphs = {
    display: displayVolumeGlyph,
    volume0: rocket_nuimo_1.emptyGlyph,
    volume1: volume1,
    volume2: volume2,
    volume3: volume3,
    volume4: volume4,
    volume5: volume5,
    volume6: volume6,
    volume7: volume7,
    volume8: volume8,
    volume9: volume9,
    volume10: volume9,
};
