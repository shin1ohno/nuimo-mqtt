...
const node_roon_api_transport_1 = __importDefault(
  require("node-roon-api-transport")
);
const node_roon_api_settings_1 = __importDefault(
  require("node-roon-api-settings")
);
...
class BootstrapCore {
  ...
  initiateSubscription() {
    return new rxjs_1.Observable((subscriber) => {
      configStore_1.ConfigStore.loadBootstrapPluginProps().then((conf) =>
  ...
  exposeNuimoToSetting(nuimo) {
    return configStore_1.ConfigStore.loadRoonConfig()
      ...
      .then((newConfig) => {
        configStore_1.ConfigStore.saveRoonConfig(newConfig).catch((e) =>
          nuRoon_1.logger.warn(e)
        );
  ...
  roonSettings(roon) {
    ...
    const roonApiSettings = new node_roon_api_settings_1.default(roon, {
      get_settings: (callback) => {
        configStore_1.ConfigStore.loadRoonConfig().then((config) =>
      ...
      save_settings: (req, dry_run, settings) => {
        ...
        if (!dry_run && !l.has_error) {
          configStore_1.ConfigStore.saveRoonConfig(
            ...
            Object.entries(l.values)
              ...
              .forEach((setting) => {
                const n = bootstrapManager_1.BootstrapManager.find(this, {
                  id: setting.id,
                });
                if (!n) return;
